...
class DOAM(nn.Module):
    def __init__(self):
        ...
        self.conv2d.add_module('conv2d', nn.Conv2d(4, 3, kernel_size=1))
    ...
    def RIA(self,x):
        ...
        og_x = x
        refine_30 = []
        x_pooled_upsample_5  = torch.zeros((x.shape[0],8,1024,1024)).to(device)
        x_pooled_upsample_10 = torch.zeros((x.shape[0],8,1024,1024)).to(device)
        x_pooled_upsample_15 = torch.zeros((x.shape[0],8,1024,1024)).to(device)
    ...
    def forward(self,im):
        ...
        x3 = Variable(im[:, 2].unsqueeze(1)).to(device)
        weight_hori = Variable(self.weight_hori).to(device)
        weight_vertical = Variable(self.weight_vertical).to(device)
